var chatbot={
	...
	chercher : function(recherche){
		for (i = 0; i < this.dialogue.length; i++) {
			for (j = 0; j < this.dialogue[i].declencheur.length; j++) {
				var reg=RegExp(this.dialogue[i].declencheur[j]);
					if(reg.test(recherche)&&this.si(i)&&this.sauf(i)){
						var rand;
						if(Array.isArray(this.dialogue[i].reponse))
							rand = this.dialogue[i].reponse[Math.floor(Math.random() * this.dialogue[i].reponse.length)];
						else  
							rand=this.dialogue[i].reponse;
