//enregistrer les données du formulaire
	function chargerboite(){
		if(selection!=null){
			...
			if($('form #nom').val())
			  charge.nom=$('form #nom').val();
			else
			  charge.nom=null;
			if($('form #couleur').val())
			  charge.couleur=$('form #couleur').val();
			else
			  charge.couleur=null;
			...
			if($('form #drapeau').val())
			  ...
			  charge.drapeau=null;
			if($('form #si').val())
			  charge.si=$('form #si').val();
			else
			  charge.si=null;
			if($('form #operation').val())
			  charge.operation=$('form #operation').val();
			else
			  charge.operation=null;
			if($('form #lien').val())
			  charge.lien=$('form #lien').val();
			else
			  charge.lien=null;
			if($('form #variable').val())
			  charge.variable=$('form #variable').val().split(separateur);
			else
			  charge.variable=null;
			if($('form #son').val())
			  charge.son=$('form #son').val();
			else
			  charge.son=null;
			if($('form #bruit').val())
			  charge.bruit=$('form #bruit').val();
			else
			  charge.bruit=null;;
			if($('form #image').val())
			  charge.image=$('form #image').val();
			else
			  charge.image=null;
			if($('form #tag').val())
			  charge.tag=$('form #tag').val();
			else
			  charge.tag=null;
...
//afficher les données de la boite sélectionnée dans le formulaire
	function chargerformulaire(){
		...
		if(selection!=null&&selection.length){
			...
			if (mode!="suite"&&mode!="loop") {mode="random"}
...
//charger la liste des liens sélectionnables dans l'input
	function listeliens(){
		...
		$("form #lienlist").empty();
		$("form #lienlist").append($("<option>").attr('value',"retour").text("retour"));
	...
	function testercondition(){
		...
		try {
			if(eval(condi)!=true&&eval(condi)!=false)
				...
				$("form #si").nextAll(".error:first").hide();
		}
		catch(error) {
	...
	function testeroperation(){
		...
		if(nomVars.length){
			for (var search = 0; search < nomVars.length; search++) {
				...
				if(variables[nomVar.entre]==null)
					variables[nomVar.entre]=0;
			}
		}
		try {
			eval(op)
