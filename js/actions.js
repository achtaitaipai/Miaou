...
function chargerboite() {
	if (selection != null) {
		...
		if ($('form #declencheur').val()) charge.declencheur = $('form #declencheur').val().split(separateur)
		else charge.declencheur = ''
		if ($('form #reponse').text()) charge.reponse = $('form #reponse').text().split(separateur)
		else charge.reponse = null
		if ($('.check-boxes input:checked').val()) charge.mode = $('.check-boxes input:checked').val()
		else charge.mode = null
		if ($('form #drapeau').val()) charge.drapeau = $('form #drapeau').val().split(separateur)
		else charge.drapeau = null
		if ($('form #si').val()) charge.si = $('form #si').val()
		else charge.si = null
		if ($('form #operation').val()) charge.operation = $('form #operation').val()
		else charge.operation = null
		if ($('form #lien').val()) charge.lien = $('form #lien').val()
		else charge.lien = null
		if ($('form #variable').val()) charge.variable = $('form #variable').val().split(separateur)
		...
		//changer la couleur
		if (donnees.lireboite()[i].couleur != '#8f88f9') selection.css('color', donnees.lireboite()[i].couleur)
...
function suprimerboite() {
	if (selection != null) {
		...
		if (index == donnees.lireboite().length && $('#listeboites li').eq(index - 1).length) {
			selection = null
...
function representerboite(rep, boite) {
	...
	if (boite.drapeau == null && boite.variable == null) rep.children('.icon-flag').hide()
...
function affichercouleur() {
	...
	$(".couleurs div[data-col='" + $('#couleur').val() + "']").toggleClass('select')
}
...
function listeliens() {
	...
	$('form #lienlist').empty()
	$('form #lienlist').append($('<option>').attr('value', 'return').text('return'))
	$('form #lienlist').append($('<option>').attr('value', 'home').text('home'))
	for (var i = 0; i < liste.length; i++) {
		$('form #lienlist').append($('<option>').attr('value', liste[i].identifiant).text(liste[i].nom))
	}
}
...
function testercondition() {
	...
	try {
		if (eval(condi) != true && eval(condi) != false) console.log("la condition n'en est pas une")
