var miaou = {
	...
	conditionner: function (condition) {
		...
		if (nomVars != null) {
			for (var search = 0; search < nomVars.length; search++) {
				...
				if (Var == null) Var = 0
				if (typeof Var === 'string' || Var instanceof String) Var = '"' + Var + '"'
	...
	operations: function (index) {
		...
		if (nomVars.length) {
			for (var search = 0; search < nomVars.length; search++) {
				...
				if (miaou.variables[nomVar.entre] == null) miaou.variables[nomVar.entre] = 0
			}
		}
		try {
			eval(op)
		} catch (error) {
			console.error("erreure dans l'opération' de la boite :" + miaou.dialogue[index].nom)
			console.error(error)
			return true
		}
	},
	...
	predef: function (recherche) {
		...
		//delai secondes
		miaou.variables['DURATION'] = Math.floor((Date.now() - miaou.debutemps) / 1000)
		miaou.variables['DELAY'] = Math.floor((Date.now() - miaou.derniererequete) / 1000)
		miaou.derniererequete = Date.now()
