...
function ouvrir(fichier) {
  ...
  if (par != false) parametres = JSON.parse(par)[0];
  selection = null;
  actulisteboites();
...
function exporter() {
  ...
  var expor = remplacermarqueur(
    expor,
    "//tablo",
    "var dialogues = " + tablo + "//fintableau"
  );
  var expor = remplacermarqueur(
    expor,
    "//par",
    "var parametres = [" + par + "]//finpar"
  );
  var expor = remplacermarqueur(expor, "//miaou", miaoujs);
  var titre = "no-title";
  if (parametres.title != null && parametres.title != "")
    titre = parametres.title;
...
function remplacermarqueur(template, marker, text) {
  ...
  return (
    template.substr(0, markerIndex) +
    text +
    template.substr(markerIndex + marker.length)
  );
}
...
function remplacerentre(template, marker1, marker2, remplacent) {
  ...
  if (i1 == -1 || i2 == -1) {
    ...
    i1 += marker1.length;
    var result =
      txt.substring(0, i1) + remplacent + txt.substring(i2, txt.length);
    return result;
  }
}
